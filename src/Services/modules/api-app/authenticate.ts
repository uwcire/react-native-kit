import { TypeLoginRequest, TypeSignUp } from '@/services/interface/authenticate'
...
export const register = (params: TypeSignUp) =>
  request.post(`auth/register`, params)
export const forgotPassword = (email: string) =>
  request.post(`auth/forgot-password`, { email })
export const checkIsExistEmail = (email: string) =>
  request.post(`auth/check-account-existed`, { email })
export const getVerifyCode = (email: string) =>
  request.post(`auth/request-verified-code`, { email })
